define(function (require, exports, module) {
    ...
    exports.init = function () {
        $('#pair .rightUser img,#pair .leftUser img').attr('src', $('#config>#FileWebHost').val() + '/ScreenTheme/default/images/default_pair.jpg');
    ...
    var GetPairs = function () {
        $('#pairUl').html('');
        $.extendGetJSON(moduleCommon.httpURL + $("#GetPairs").val(), {}, function (data) {
            if (data.length > 0) {
                $(data).each(function (index, element) {
                    $("#pairUl").prepend('<li data-pair="1"><span></span><div><img onerror="imgError(this);" src="' + element.OneHead + '"><font>' + element.OneNickName + '</font></div><div><img onerror="imgError(this);" src="' + element.AnotherHead + '"><font>' + element.AnotherNickName + '</font></div><i class="hasSubmit">已提交</i></li>');
                });
    ...
    var GetPairFans = function () {
        ...
        leftUser.html('').hide();
        rightUser.html('').hide();
        $.extendGetJSON(moduleCommon.httpURL + $("#GetPairFans").val(), {}, function (data) {
            if (data.length > 0) {
                $(data).each(function (index, element) {
                    var str = '<li data-userid="' + element.Id + '" data-headpath="' + element.HeadPath + '"><img onerror="imgError(this);" src="' + element.Head + '"><br><span>' + element.NickName + '</span></li>';
                    if (index % 2 == 0) {
                        leftUser.append(str);
                    } else {
    ...
    var SubmitPairFans = function (v) {
        ...
        if (!$("#submitPair").hasClass("gray") && submitCount > 0) {
            ...
            var submitForm = $('<form/>');
            $("#pairUl li[data-pair!=1]").each(function (index, element) {
                submitForm.append('<input name="[' + index + '].OneFansId" type="hidden" value="' + $(element).data('onefansid') + '" />');
                submitForm.append('<input name="[' + index + '].OneNickName" type="hidden" value="' + $(element).data('onefansnickname') + '" />');
                submitForm.append('<input name="[' + index + '].OneHead" type="hidden" value="' + $(element).data('onefansheadpath') + '" />');
                submitForm.append('<input name="[' + index + '].AnotherFansId" type="hidden" value="' + $(element).data('anotherfansid') + '" />');
                submitForm.append('<input name="[' + index + '].AnotherNickName" type="hidden" value="' + $(element).data('anotherfansnickname') + '" />');
                submitForm.append('<input name="[' + index + '].AnotherHead" type="hidden" value="' + $(element).data('anotherfansheadpath') + '" />');
            });
            submitForm.append('<input name="isPeng" type="hidden" value="false" />');
            $.extendPost(moduleCommon.httpURL + $("#SubmitPairs").val(), submitForm.serializeArray(), "json", function (data) {
                moduleCommon.loaded();
    ...
    var showPairAnimate = function (OneFansHead, OneFansNickName, AnotherFansHead, AnotherFansNickName) {
        ...
        setTimeout(function () {
            ...
            if (leftUl.find('li').size() == 0) {
                leftUl.parent().append('<div><img src="' + $('#config>#FileWebHost').val() + '/ScreenTheme/default/images/default_pair.jpg"><br><span id="defaultFemale">没人了</span></div>');
            }
            if (rightUl.find('li').size() == 0) {
                rightUl.parent().append('<div><img src="' + $('#config>#FileWebHost').val() + '/ScreenTheme/default/images/default_pair.jpg"><br><span id="defaultMale">没人了</span></div>');
            }
            if (selectNumber > 1) {
                selectNumber--;
                beginPair(); //重新抽奖
